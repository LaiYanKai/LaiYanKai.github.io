...
Algs.R2P = class extends Algs.AbstractAlg {
  ...
  constructor(alg_params) {
    ...
    this.#canvas_trace = new Algs.R2PCanvasTrace(0);
    this.#canvas = new Algs.R2PCanvasLinks(1);

    this.setCanvases([this.#canvas_trace, this.#canvas]);

    // Set up lenses
    const lenses = [new UI.LensNone(this.#canvas, "None", "None")];
  ...
  #initial(start_coord, goal_coord) {
    ...
    if (cast_result === null) {
      ...
      this.#step.registerWithData(sprite, R2PActionLink.Display, true);
      const cost = Utils.euclidean(
        Utils.subtractCoords(this.#goal_coord, this.#start_coord)
      );
      this.#step.registerWithData(sprite, R2PActionLink.Cost, cost);
      this.#step.registerWithData(sprite, R2PActionLink.Knob, 0);
      this.#step.registerWithData(sprite, R2PActionLink.LeftRay, null);
      this.#step.registerWithData(sprite, R2PActionLink.RightRay, null);
      this.#step.registerWithData(sprite, R2PActionLink.ProgRay, null);
      this.#step.registerWithData(sprite, R2PActionLink.Side, R2PSide.L);
      this.#step.registerWithData(
        sprite,
        R2PActionLink.SrcCoord,
        this.#start_coord.slice()
      );
      this.#step.registerWithData(
        sprite,
        R2PActionLink.Status,
        R2PLinkStatus.Path
      );
      this.#step.registerWithData(sprite, R2PActionLink.Tdir, R2PTreeDir.Tgt);
      this.#step.registerWithData(
        sprite,
        R2PActionLink.TgtCoord,
        this.#goal_coord.slice()
      );
      this.#step.registerWithData(sprite, R2PActionLink.Type, R2PLinkType.Vy);
      this.#step.registerWithData(sprite, R2PActionLink.ZIndex, 0);
      this.#closeStep();
      return true;
    }
    ...
    const [left_col_pose, right_col_pose, col_coord] = cast_result;
    const left_trace_crn = new Algs.R2PCorner(
      R2PSide.L,
      left_col_pose.didx,
      left_col_pose.coord,
      left_col_pose.convex,
      -1
    );
    const left_in_map = left_trace_crn.didx !== 0;
    const right_trace_crn = new Algs.R2PCorner(
      R2PSide.R,
      right_col_pose.didx,
      right_col_pose.coord,
      right_col_pose.convex,
      -1
    );
    const right_in_map = right_trace_crn.didx !== 0;

    // -------------- initialize rays -------------------------
    const ray_diff = Utils.subtractCoords(this.#goal_coord, this.#start_coord);
    const ray = this.#createRay(ray_diff, true, true);
    const rev_ray_diff = Utils.subtractCoords(
      this.#start_coord,
      this.#goal_coord
    );
    const rev_ray_opened = this.#createRay(rev_ray_diff, false, true);
    const rev_ray_closed = this.#createRay(rev_ray_diff, true, true);
    const deep_start = Utils.addCoords(
      start_coord,
      Utils.unitCoord(rev_ray_diff)
    );
    const deep_goal = Utils.addCoords(goal_coord, Utils.unitCoord(ray_diff));
    ...
    if (left_in_map) {
      ...
      left_tr = this.#createTrace(
        left_trace_crn,
        col_coord,
        R2PTraceClass.Inactive
      );
    } else {
      const left_trace_sprite = this.#createTraceSprite(
        col_coord,
        left_trace_crn.coord,
        R2PTraceClass.Inactive
      );
      this.#newSmallStep();
      this.#removeTraceSprite(left_trace_sprite);
    }
    ...
    if (right_in_map) {
      ...
      right_tr = this.#createTrace(
        right_trace_crn,
        col_coord,
        R2PTraceClass.Inactive
      );
    } else {
      const right_trace_sprite = this.#createTraceSprite(
        col_coord,
        left_trace_crn.coord,
        R2PTraceClass.Inactive
      );
      this.#newSmallStep();
      this.#removeTraceSprite(right_trace_sprite);
    }
  ...
  run(start_coord, goal_coord) {
    ...
    if (this.#initial(start_coord, goal_coord)) return this.#path;
    this.#dbg("initial done");
    ...
    while (!this.#open_list.isEmpty()) {
      ...
      this.#dbg(`polled qnode ${qnode}`);
      this.#dbg(`polled qnode link: ${qnode.link}`);

      if (qnode.type === R2PQueueType.Cast) {
        if (this.#caster(qnode)) break;
  ...
  #atCrn(test_coord, test_didx, at_crn) {
    return (
      Utils.equalIntegerCoords(test_coord, at_crn.coord) &&
  ...
  #closeStep() {
    if (this.#step) {
      ...
      let sprites = new Set();
      for (const act of this.#step.actions()) sprites.add(act.sprite);
      for (const sprite of sprites) sprite.vis();
  ...
  #trace(tr) {
    ...
    const next_pose = this.#mapper.trace(
      trace_crn.side,
      trace_crn.coord,
      trace_didx
    );
    ...
    this.#step.registerWithData(tr.sprite, R2PActionTrace.Add, trace_crn.coord);

    const out_of_map = trace_crn.didx === 0;
    if (out_of_map) {
      this.#dbg("[TR] At map boundary!");
    }
   ...
   * If collided, returns the poses for the first and right corners of the trace and the collision point.
   * If reached, null is returned.
   ...
   * Returns the poses for the first and right corners of the trace and the collision point.
   * @param {Algs.R2PCorner} from_crn
   * @param {[number, number]} diff
   * @returns { [Algs.R2PPose, Algs.R2PPose, [number, number]]}
   */
  #project(from_crn, diff) {
    const result = this.#mapper.project(from_crn.coord, diff);
    return result;
  }
  ...
  #overlapRule() {
    ...
    for (const buffer_crn of this.#overlap_buffer) {
      ...
      if (other_crn) this.#overlapRuleGotoSrcVyEyFromCrn(buffer_crn);
    }
  ...
  #overlapRuleConvToEyForVyLink(tdir, link, root_link) {
    ...
    for (let i = 0; i < link.numNeighbors(-tdir); ) {
      ...
      if (!this.#overlapRuleConvToEyForVyLink(tdir, leaf_link, link)) ++i; // If not erased, ++i. If erased, i stays the same.
    }
  ...
  #overlapRuleGotoSrcVyEyFromCrn(crn) {
    let found_links = [
      ...
      crn.findLink(R2PTreeDir.Src, R2PLinkType.Tm),
    ];

    // ======= For each SrcVu or SrcVy link still in the corner ======,
    while (
      found_links[0] !== undefined ||
      found_links[1] !== undefined ||
      found_links[2] !== undefined
    ) {
      for (let i = 0; i < 3; ++i) {
        if (found_links[i] !== undefined) {
          ...
          found_links[i] = crn.findLink(
            R2PTreeDir.Src,
            i === 0 ? R2PLinkType.Vu : i === 1 ? R2PLinkType.Eu : R2PLinkType.Tm
          );
        }
      }
    }
  ...
  #overlapRuleConvertToTgtTree(link) {
    ...
    if (link.tdir === R2PTreeDir.Tgt) return;
  ...
  #casterReachedFoundPath(cast_link) {
    ...
    while (1) {
      ...
      if (link.crn === this.#start_crn) break;
      this.#step.registerWithData(
        link.sprite,
        R2PActionLink.Status,
        R2PLinkStatus.Path
    ...
    while (1) {
      ...
      if (link.crn === this.#goal_crn) break;
      this.#step.registerWithData(
        link.sprite,
        R2PActionLink.Status,
        R2PLinkStatus.Path
    ...
    this.#dbg("found path ", path.map((coord) => coord.join(",")).join("; "));
  }
  ...
  #casterReachedTm(cast_link) {
    ...
    // =========== Merge Ray =============
    const cast_ray_closed = this.#createRay(src_to_tgt, true, true);
    ...
    this.#dbg(`[CTm] Can place pt at reached Tm.`);
    for (let i = 0; i < cast_link.numNeighbors(R2PTreeDir.Tgt); ++i) {
      const tgt_link = cast_link.neighborAt(i);
      const is_vis =
        tgt_crn.side * Utils.detCoords(tgt_edge, tgt_link.getDiff()) >= 0;
      if (is_vis) {
        tgt_links.push(tgt_link);
        this.#dbg(`[CTm] Can cast from new pt @ reached Tm for ${tgt_link}`);
      } else {
        this.#dbg(
          `[CTm] Not castable from new pt @ reached Tm for ${tgt_link}`
        );
      }
    }
    ...
    if (tgt_links.length > 0) {
      this.#dbg(`[CTm] Prepare for castable tgt links`);
      ...
      this.#changeLink(
        ...
        cast_link.right_ray,
        null,
        cast_link.copyNeighbors(R2PTreeDir.Src),
        tgt_links,
        false,
        R2PListDir.Back
      );
      ...
      if (
        ...
        this.#dbg(`[CTm] Call CumulativeVis for new castable link`);
        this.#casterReachedWithCumVis(new_cast_link);
      } else {
        this.#dbg(`[CTm] Call NonCumulativeVis for new castable link`);
    ...
    // Trace from cast_link if have tgts (non-castable ones)
      ...
      this.#dbg(`[CTm] Prepare trace for reached Tm for non-castable links`);
      this.#setupTracerFromLink(cast_link);
    }
  }
  ...
  #casterReachedWithoutCumVis(cast_link) {
    ...
    // ====== merge ray =========
    const cast_ray_diff = Utils.subtractCoords(tgt_crn.coord, src_crn.coord);
    const cast_ray_closed = this.#createRay(cast_ray_diff, true, true);
    const cast_ray_opened = this.#createRay(cast_ray_diff, false, false);
    this.#mergeRay(-tgt_crn.side, cast_link, cast_ray_closed);

    // isolate all tgt links to merge ray
    const tgt_links = cast_link.copyNeighbors(R2PTreeDir.Tgt);
    for (const tgt_link of tgt_links) {
      const new_tgt_link = this.#isolateLink(
        R2PTreeDir.Src,
        tgt_link,
        cast_link
      );
    ...
    let num_links_at_crn =
      tgt_crn.numLinks(R2PTreeDir.Src) + tgt_crn.numLinks(R2PTreeDir.Tgt);
    ...
    if (num_links_from_cast === num_links_at_crn) {
      ...
      if (other_crn)
        num_links_at_crn +=
          other_crn.numLinks(R2PTreeDir.Src) +
          other_crn.numLinks(R2PTreeDir.Tgt);
    }
  ...
  #casterReachedWithCumVis(cast_link) {
    ...
    this.#dbg(`[CR] Cast reached with CV for cast link: ${cast_link}`);
    ...
    const prev_first_link =
      next_tdir === R2PTreeDir.Src ? first_tgt_link : src_link;
    const next_first_link =
      next_tdir === R2PTreeDir.Src ? src_link : first_tgt_link;

    // =========== Discard if prev corner is Ey and reached corner is opposite side ==============
    if (
      prev_first_link.type === R2PLinkType.Ey &&
    ...
    if (Utils.approxGt(cost_at_next, best_at_next.min_cost)) {
      ...
      const can_discard =
        best_at_next.to_cur &&
        next_tdir *
          next_first_link.crn.side *
          Utils.detCoords(prev_to_next, best_at_next.to_cur) >
          0;
      if (can_discard) {
        this.#disconnectLink(R2PTreeDir.Src, cast_link, src_link);
        this.#eraseTree(R2PTreeDir.Tgt, cast_link);
    ...
    // ================ Cheapest so far to reach next crn from cast link ===============
      ...
      const best_at_other = other_next_crn
        ? other_next_crn.best(-next_tdir)
        : null;
      ...
      const replace_to_cur =
        !best_at_next.to_cur ||
        next_tdir *
          next_first_link.crn.side *
          Utils.detCoords(best_at_next.to_cur, prev_to_next) >
          0;
      if (replace_to_cur) {
        // replace best.to_cur if it is likely to reject more future searches
      ...
      if (
        best_at_other &&
        Utils.approxGt(cost_at_next, best_at_other.min_cost)
      ) {
      ...
      // ----------- Cheaper or eq cost as the other side's best ----------
        ...
        if (best_at_other) best_at_other.min_cost = cost_at_next;
    ...
    if (next_tdir === R2PTreeDir.Tgt) {
      // merge rays
      const cast_ray_opened = this.#createRay(prev_to_next, false, false);
      const cast_ray_closed = this.#createRay(prev_to_next, true, true);
      ...
      const new_src_link = this.#isolateLink(
        R2PTreeDir.Tgt,
        src_link,
        cast_link
      );
      this.#changeLink(
        ...
        R2PLinkType.Vu,
        R2PTreeDir.Tgt,
        new_src_link.left_ray,
        new_src_link.right_ray,
        null,
        undefined,
        undefined,
        true,
  ...
  #casterCollidedThirdTrace(cast_link) {
    ...
    const third_pose = this.#mapper.trace(
      trace_side,
      src_crn.coord,
      trace_didx
    );
    if (third_pose.didx === 0) {
      const sprite = this.#createTraceSprite(
        src_crn.coord,
        third_pose.coord,
        R2PTraceClass.Inactive
      );
      this.#newSmallStep();
      this.#removeTraceSprite(sprite);
      return null;
    }

    // check if reverse trace is at map boundary
    const rev_trace_didx = this.#getPrevTraceDirIdx(src_crn);
    const rev_pose = this.#mapper.trace(
      -trace_side,
      src_crn.coord,
      rev_trace_didx
    );
    ...
    // init trace crn
    const trace_crn = new Algs.R2PCorner(
      third_pose.side,
      third_pose.didx,
      third_pose.coord,
      third_pose.convex,
    ...
    const tgt_crn = cast_link.neighborAt(R2PTreeDir.Tgt, 0).crn;
    const cast_ray_diff = Utils.subtractCoords(tgt_crn.coord, src_crn.coord);
    const cast_ray = this.#createRay(cast_ray_diff, true, true);
    ...
    this.#changeLink(
      ...
      null,
      [],
      cast_link.copyNeighbors(R2PTreeDir.Tgt),
      true,
      R2PListDir.Auto
    );
    ...
    const tgt_prog_ray = Utils.subtractCoords(trace_crn.coord, src_crn.coord);
    this.#changeLink(
      new_tgt_link,
      trace_crn,
      R2PLinkType.Tm,
      ...
      null,
      tgt_prog_ray,
      // [], [new_oc_link], NaN, R2PListDir.Back);
      [],
      [new_un_link],
      NaN,
      R2PListDir.Back
    ...
    return this.#createTrace(trace_crn, src_crn.coord, R2PTraceClass.Inactive); // any cast_diff that is not cardinal will do.
  }
   ...
   * @param {Algs.R2PPose} first_pose
   * @param {[number, number]} col_coord
   * @returns {null | Algs.R2PTrace}
   */
  #casterCollidedMjrMnrTrace(cast_link, trace_side, first_pose, col_coord) {
    this.#newSmallStep();

    // return if at map boundary
    if (first_pose.didx === 0) {
      const sprite = this.#createTraceSprite(
        first_pose.coord,
        col_coord,
        R2PTraceClass.Inactive
    ...
    const cast_ray_diff = Utils.multiplyCoord(cast_link.getDiff(), -1);
    const cast_ray = this.#createRay(cast_ray_diff, true, true);
    ...
    // generate tgt link
    const new_tgt_link = this.#createLink();
    const tgt_prog_ray = [-cast_ray_diff[0], -cast_ray_diff[1]];
    this.#changeLink(
      new_tgt_link,
      trace_crn,
      R2PLinkType.Tm,
    ...
    return this.#createTrace(trace_crn, col_coord, R2PTraceClass.Inactive);
  }

  /**
   * Generates trace from collisions
   * @param {Algs.R2PPose} left_first_pose
   * @param {Algs.R2PPose} right_first_pose
   * @param {[number, number]} col_coord
   * @param {Algs.R2PLink} cast_link
   */
  #casterCollided(left_first_pose, right_first_pose, col_coord, cast_link) {
    ...
    let mnr_first_pose, mjr_first_pose;
    if (mjr_side === R2PSide.L) {
      mjr_first_pose = left_first_pose;
      mnr_first_pose = right_first_pose;
    } else {
      mjr_first_pose = right_first_pose;
      mnr_first_pose = left_first_pose;
    }
    ...
    if (cast_link.neighborAt(R2PTreeDir.Src, 0).type !== R2PLinkType.Ey) {
      ...
      mnr_trace = this.#casterCollidedMjrMnrTrace(
        cast_link,
        mnr_side,
        mnr_first_pose,
        col_coord
    ...
    const mjr_trace = this.#casterCollidedMjrMnrTrace(
      cast_link,
      mjr_side,
      mjr_first_pose,
      col_coord
    ...
    // Generate traces
    if (mnr_trace) {
      this.#dbg(`[CC] MnrTrace for ${dbg_str}`);
      this.#tracer(mnr_trace);
    }
    if (!mnr_trace || !mnr_trace.refound_src) {
      if (third_trace) {
        this.#dbg(`[CC] ThirdTrace for ${dbg_str}`);
        this.#tracer(third_trace);
      }
    } else {
      // refound src, no need to do third trace
      if (third_trace) {
        // delete links in third trace
        this.#eraseTree(
          R2PTreeDir.Src,
          third_trace.crn.linkAt(R2PTreeDir.Src, 0)
        );
        this.#eraseTree(
          R2PTreeDir.Tgt,
          third_trace.crn.linkAt(R2PTreeDir.Tgt, 0)
        );
        // remove trace for vis
        this.#removeTraceSprite(third_trace.sprite);
      }
    }
    if (mjr_trace) {
      this.#dbg(`[CC] MjrTrace for ${dbg_str}`);
  ...
  #mergeRay(side, link, new_ray) {
    ...
    const can_replace =
      old_ray === null ||
      side * Utils.detCoords(old_ray.getCoord(), new_ray.getCoord()) > 0;
    if (can_replace) {
      this.#dbg(
        `[MR] Replace Old${side}Ray(${old_ray}) with new ray (${new_ray}) for link ${link}`
      );
      if (side === R2PSide.L) {
        link.left_ray = new_ray.copy();
        this.#step.registerWithData(
          link.sprite,
          R2PActionLink.LeftRay,
          new_ray.copy()
        );
      } else {
  ...
  #setupTracerFromLink(src_link) {
    ...
    for (const tgt_link of tgt_links) {
      const new_tgt_link = this.#isolateLink(
        ...
        tgt_link,
        src_link
      ); // make sure to point only to src link
    ...
    this.#changeLink(
      ...
      NaN,
      R2PListDir.Back
    ); // there should not be any tgt links in src link

    // ----- Trace -----------
    const tr = this.#createTrace(trace_crn, null, R2PTraceClass.Active);
    this.#tracer(tr);
  }
  ...
  #tracer(tr) {
    ...
    this.#newMedStep();
    this.#setTraceSpriteClass(tr.sprite, R2PTraceClass.Active);
    while (1) {
      this.#dbg(`[TR] At ${tr.crn} ======`);

      tr.all_src_prog = true;
      tr.all_tgt_prog = true;

      // ========== Skip if refound src corner ==============
      if (this.#tracerRefoundSrcRoot(tr)) break;
      // ========== Process src link ===================
      if (this.#tracerProcess(tr, R2PTreeDir.Src)) break; // no more src links
      // ========== Process tgt link ===================
      if (this.#tracerProcess(tr, R2PTreeDir.Tgt)) break; // no more tgt links

      // ========== Interrupt ==================
      if (this.#tracerInterruptRule(tr)) break;

      // ========== Place rule ================
      if (this.#tracerPlaceRule(tr)) break;
    ...
    for (const tdir of [R2PTreeDir.Src, R2PTreeDir.Tgt]) {
      ...
      for (const link of links) this.#eraseTree(tdir, link);
    }

    // remove trace for vis
    this.#removeTraceSprite(tr.sprite);
  }

  #addToOverlapBuffer(crn) {
    this.#dbg(`[OB] Add to overlap buffer for ${crn}`);
    if (!this.#overlap_buffer.includes(crn)) this.#overlap_buffer.push(crn);
  }
  ...
  #tracerPlaceRule(tr) {
    ...
    if (trace_crn.convex) {
      ...
      if (!src_link.is_prog) return false;

      // ---------- Cannot place if next trace dir does not decrease src prog ---------------
      const next_trace_dir = Utils.dirIndexToDir(
        this.#getNextTraceDirIdx(trace_crn)
      );
      const is_rev =
        trace_crn.side * Utils.detCoords(src_link.prog_ray, next_trace_dir) >=
        0;
      if (!is_rev) return false;

      // ----------- Re-anchor src link to turning point ------------
      const place_crn = this.#corners.at(
        trace_crn.side,
        trace_crn.didx,
        trace_crn.coord,
        trace_crn.convex
      ...
      const placed_link_type =
        ssrc_link.type === R2PLinkType.Eu || ssrc_link.type === R2PLinkType.Ey
          ? R2PLinkType.Eu
          : R2PLinkType.Vu;
      this.#changeLink(
        src_link,
      ...
      if (!tr.has_overlap && placed_link_type !== R2PLinkType.Eu) {
        let num_links_at_place =
          place_crn.numLinks(R2PTreeDir.Src) +
          place_crn.numLinks(R2PTreeDir.Tgt);
        ...
        if (tr.has_overlap) {
          this.#dbg(`Has overlap at ${place_crn}`);
        }
      }
      ...
      for (const tgt_link of tgt_links) {
        const is_vis =
          tgt_link.is_prog &&
          trace_crn.side * Utils.detCoords(next_trace_dir, tgt_link.prog_ray) >=
            0;
        // -------------- Is Castable -----------------
        if (is_vis) {
          this.#changeLink(
      ...
      if (trace_crn.numLinks(R2PTreeDir.Tgt) === 0) return true; // all tgt links are castable.
  ...
  #tracerInterruptRule(tr) {
    if (
      tr.num_crns < this.#num_interrupt ||
      ...
      for (const tgt_link of tgt_links) {
        ...
        if (min_tgt_cost > tgt_link.cost) min_tgt_cost = tgt_link.cost;
      }
  ...
  #tracerProcess(tr, tdir) {
    ...
    for (let link_idx = 0; link_idx < trace_crn.numLinks(tdir); ++link_idx) {
      ...
      // ========== Progression Rule ==============
      if (this.#tracerProgRule(tr, link)) continue;
      ...
      if (this.#tracerIsStartOrGoal(link)) continue;

      // ========== OcSec ================
      if (this.#tracerOcSecRule(tr, link)) continue;
  ...
  #tracerPruneRule(tr, link) {
    ...
    if (root_crn.side !== tr.crn.side) return false;

    const root_to_cur = link.getDiff();

    let root_links = link.copyNeighbors(link.tdir);
    for (const root_link of root_links) {
      ...
      if (
        ...
        this.#dbg(`[TR] Prepare to Prune ${link}`);

        // -------- Disconnect root link and isolate ---------
        this.#disconnectLink(link.tdir, link, root_link);
        const new_link = this.#isolateLink(-link.tdir, root_link, null);

        // ------------ Change the new link ----------------
        const left_ray =
          link.tdir === R2PTreeDir.Tgt ? null : new_link.left_ray;
        const right_ray =
          link.tdir === R2PTreeDir.Tgt ? null : new_link.right_ray;
        const prog_ray = Utils.subtractCoords(
          tr.crn.coord,
          new_link.getRootCrn().coord
        );
        this.#changeLink(
          new_link,
          tr.crn,
    ...
    // ======== Remove link if root links have been erased ==========
    if (link.numNeighbors(link.tdir) === 0) {
      this.#eraseLink(link);
      return true;
    } else {
      return false;
  ...
  #tracerOcSecRule(tr, link) {
    ...
    if (root_crn.side === trace_crn.side) return false;

    const root_to_cur = link.getDiff();

    if (
      root_link.type === R2PLinkType.Oc ||
      root_link.type === R2PLinkType.Un
    ) {
      ...
      if (root_crn.side * Utils.detCoords(ttgt_to_tgt, root_to_cur) > 0) {
        this.#dbg(
          `[TR] Prepare to erase bcos 180 around TTgtOcLink ${root_link}`
        );
        ...
        // in oc sec of nb_link.
        this.#dbg(`[TR] In OcSec of TgtOcLink ${root_link}`);
        return false;
      }
    }
    ...
    if (in_ocsec) {
      ...
      if (link.tdir === R2PTreeDir.Src) {
        this.#dbg(
          `[TR] Prepare to recurse bcos In OcSec of SSrclink  ${root_link}`
        );
        // Generate ROS trace
        const new_trace_crn = new Algs.R2PCorner(
          new_trace_pose.side,
          new_trace_pose.didx,
          new_trace_pose.coord,
          new_trace_pose.convex,
          -1
        );

        // move the src link (link) to the new trace corner
        const src_prog_ray = Utils.subtractCoords(
          new_trace_crn.coord,
          root_crn.coord
        ); // conincident to trace_dir
        this.#changeLink(
          link,
          new_trace_crn,
          R2PLinkType.Tm,
        ...
        const new_tgt_link = this.#createLink();
        const tgt_prog_ray = Utils.subtractCoords(
          new_trace_crn.coord,
          tm_crn.coord
        );
        this.#changeLink(
        ...
        // begin trace
        this.#dbg(`[TR] Start OcSec Recurse.`);
        this.#setTraceSpriteClass(tr.sprite, R2PTraceClass.Waiting);
        const new_tr = this.#createTrace(
          new_trace_crn,
          root_crn.coord,
          R2PTraceClass.Inactive
        );
      ...
      // ========= In Ocsec of Non-Oc Tgt corner ==============
        ...
        this.#dbg(`[TR] Place Oc bcos In OcSec of TTgtLink ${root_link}`);
        // Place the tgt link at the first corner from tgt crn.
        const oc_crn = this.#corners.at(
          root_crn.side,
          new_trace_pose.didx,
          new_trace_pose.coord,
          new_trace_pose.convex
  ...
  #tracerAngSecRule(tr, src_link) {
    ...
    if (src_link.tdir !== R2PTreeDir.Src) return false;
    ...
    this.#dbg(`AS: ${trace_crn}; r${ray} ; r2c: ${root_to_cur.join(",")}`);
    if (
      this.#crossedRay(R2PTreeDir.Src, side, trace_crn, ray_diff, root_to_cur)
    ) {
      this.#dbg(
        `[TR] AngSec Crossed Ray [${ray_diff.join(
          ","
        )}] for SrcLink ${src_link}`
      );
      ...
      const [left_pose, right_pose, col_coord] = this.#project(
        ssrc_link.crn,
        ray_diff
      );
      ...
      if (!this.#atCrn(prev_pose.coord, prev_pose.didx, trace_crn)) {
        ...
        this.#dbg(`[TR] Prepare Recur AS Trace`);
        this.#newSmallStep();
        ...
        this.#changeLink(
          ...
          NaN,
          R2PListDir.Back
        ); // note the cast ray is closed at this point. (src_link.left_ray or src_link.right_ray)

        // RAS trace
        this.#dbg(`[TR] Begin Recur AS Trace`);
        this.#setTraceSpriteClass(tr.sprite, R2PTraceClass.Waiting);
        const new_tr = this.#createTrace(
          new_trace_crn,
          col_coord,
      ...
      // Discard trace if the ray was already closed
      this.#newSmallStep();

      // if (ssrc_link.numNeighbors(R2PTreeDir.Src) > 0) {
      if (ray_was_closed === false) {
        this.#dbg(`[TR] Begin Link Pruning for AS`);
        ...
        // discard trace because ray is closed
      }
      // }
      // else {
      //     // discard trace because went one round around the start
      //     this.#dbg(`[TR] No Link Pruning for AS`);
      // }
  ...
  #notProgressed(tdir, trace_crn, prog_ray, root_to_cur) {
    ...
    const nrm_det =
      tdir * trace_crn.side * Utils.detCoords(root_to_cur, prog_ray);
    return nrm_det > 0;
  }
  ...
  #tracerProgRule(tr, link) {
    ...
    if (not_prog) {
      if (link.tdir === R2PTreeDir.Src) tr.all_src_prog = false;
      else tr.all_tgt_prog = false;
      link.is_prog = false;
      this.#dbg(`[TR] Not Prog for ${link}`);
      return true;
    } else {
      // is progressed.
      if (link.tdir === R2PTreeDir.Src) {
      ...
      if (root_to_cur[0] === 0 && root_to_cur[1] === 0)
        link.prog_ray = Utils.dirIndexToDir(tr.crn.didx);
      else link.prog_ray = root_to_cur;
      link.is_prog = true;
      this.#dbg(`[TR] Prog for ${link}`);

      this.#step.registerWithData(
        link.sprite,
        R2PActionLink.ProgRay,
        link.prog_ray.slice()
      );
      return false;
    }
  ...
  #tracerProgRuleSrcProgCast(tr, src_link) {
    ...
    if (rotated_180) {
      ...
      if (
        trace_crn.numLinks(R2PTreeDir.Tgt) !== 1 ||
        trace_crn.numLinks(R2PTreeDir.Src) !== 1
      )
        throw new Error(
          `There are >1 src links or >1 tgt links for src prog cast`
        );
      ...
      // queue
      this.#dbg(`[TR] SrcProgCast for ${src_link}`);
      this.#queue(R2PQueueType.Cast, tgt_link, f);
  ...
  #setLinkCost(link, cost) {
    if (link.cost !== cost)
      this.#step.registerWithData(link.sprite, R2PActionLink.Cost, cost);
    link.cost = cost;
  }
  ...
  #calcLinkCost(link) {
    ...
    if (prev_cost !== link.cost)
      this.#step.registerWithData(link.sprite, R2PActionLink.Cost, link.cost);
  }
   ...
   */
  #queue(qtype, link, f) {
    if (link.qnode) throw new Error(`link is already queued!`);
    this.#open_list.queue(qtype, link, f);
    this.#dbg(`------- [Q] Queue ${link.qnode}`);
    this.#newSmallStep();
    this.#step.registerWithData(
      link.sprite,
      R2PActionLink.Status,
      R2PLinkStatus.Queued
    );
  }
  ...
  #createLink() {
    ...
    this.#dbg(`[CL] Created Link[${link.id}]`);
    return link;
  }
  ...
  #disconnectLinks(tdir, link, nb_links) {
    for (const nb_link of nb_links) this.#disconnectLink(tdir, link, nb_link);
  }
  ...
  #disconnectLink(tdir, link, nb_link) {
    // ======== Visualization ===========
    let action_id =
      tdir === R2PTreeDir.Src ? R2PActionLink.AddSrcId : R2PActionLink.AddTgtId;
    this.#step.registerWithData(link.sprite, action_id, [nb_link.id, false]);

    action_id =
      tdir === R2PTreeDir.Src ? R2PActionLink.AddTgtId : R2PActionLink.AddSrcId;
    this.#step.registerWithData(nb_link.sprite, action_id, [link.id, false]);
  ...
  #connectLinks(tdir, link, nb_links) {
    for (const nb_link of nb_links) this.#connectLink(tdir, link, nb_link);
  }
  ...
  #connectLink(tdir, link, nb_link) {
    ...
    let action_id =
      tdir === R2PTreeDir.Src ? R2PActionLink.AddSrcId : R2PActionLink.AddTgtId;
    this.#step.registerWithData(link.sprite, action_id, [nb_link.id, true]);
    if (link.tdir === tdir) {
      action_id =
        link.tdir === R2PTreeDir.Src
          ? R2PActionLink.SrcCoord
          : R2PActionLink.TgtCoord;
      this.#step.registerWithData(
        link.sprite,
        action_id,
        nb_link.crn.coord.slice()
      );
    }

    action_id =
      tdir === R2PTreeDir.Src ? R2PActionLink.AddTgtId : R2PActionLink.AddSrcId;
    this.#step.registerWithData(nb_link.sprite, action_id, [link.id, true]);
    if (nb_link.tdir !== tdir) {
      action_id =
        nb_link.tdir === R2PTreeDir.Src
          ? R2PActionLink.SrcCoord
          : R2PActionLink.TgtCoord;
      this.#step.registerWithData(
        nb_link.sprite,
        action_id,
        link.crn.coord.slice()
      );
    }
  ...
  #isolateLink(tdir, link, nb_link) {
    if (!nb_link) {
      if (link.numNeighbors(tdir) === 0) {
        ...
        this.#dbg(
          `[IL] Has >0 ${R2PTreeDir.toString(tdir)} for iso link ${link}`
        );
        const src_links =
          tdir === R2PTreeDir.Src ? [] : link.copyNeighbors(R2PTreeDir.Src);
        const tgt_links =
          tdir === R2PTreeDir.Src ? link.copyNeighbors(R2PTreeDir.Tgt) : [];
      ...
      if (link.numNeighbors(tdir) === 1) {
        ...
        this.#dbg(
          `[IL] Has >1 ${R2PTreeDir.toString(tdir)} for iso link ${link}`
        );
        this.#dbg(`[IL]      for nb link ${nb_link}`);
        const src_links =
          tdir === R2PTreeDir.Src
            ? [nb_link]
            : link.copyNeighbors(R2PTreeDir.Src);
        const tgt_links =
          tdir === R2PTreeDir.Src
            ? link.copyNeighbors(R2PTreeDir.Tgt)
            : [nb_link];
        const new_link = this.#createLink();
        this.#changeLink(
          new_link,
   ...
   * @param {boolean | number} calc_cost True to recalculate cost, false to keep current cost, else if number, set it to number
  ...
  #changeLink(
    ...
    const action_id =
      tdir === R2PTreeDir.Src ? R2PActionLink.TgtCoord : R2PActionLink.SrcCoord;
    this.#step.registerWithData(link.sprite, action_id, crn.coord.slice());
    this.#step.registerWithData(link.sprite, R2PActionLink.Side, crn.side);
    ...
    // ========== change type =============
    this.#step.registerWithData(link.sprite, R2PActionLink.Type, type);
    link.type = type;

    // ========== change tdir =============v
    this.#step.registerWithData(link.sprite, R2PActionLink.Tdir, tdir);
    link.tdir = tdir;
    ...
    if (left_ray) {
      ...
      this.#step.registerWithData(link.sprite, R2PActionLink.LeftRay, null);
      link.left_ray = null;
    }
    // ============= change right ray ===============
    if (right_ray) {
      ...
      this.#step.registerWithData(link.sprite, R2PActionLink.RightRay, null);
      link.right_ray = null;
    }
    // ============= change prog ray ===============
    if (prog_ray) {
      ...
      this.#step.registerWithData(link.sprite, R2PActionLink.ProgRay, null);
      link.prog_ray = null;
    }

    if (calc_cost === true) this.#calcLinkCost(link);
    else if (calc_cost === false) {
    } else this.#setLinkCost(link, calc_cost);

    this.#dbg(`[CL] Changed to ${link}`);
  }
  ...
  #eraseTree(tdir, link) {
    if (link.numNeighbors(-tdir) === 0) {
      ...
      for (const next_link of next_links) this.#eraseTree(tdir, next_link);
    }
  }
   ...
   * @returns {Algs.R2PRay}
   */
  #createRay(diff, closed, from_src) {
    return new Algs.R2PRay(diff[0], diff[1], closed, from_src);
  }

  /**
   * @param {Algs.R2PCorner} trace_crn the corner that anchors all the temporary links for tracing
   * @param {null | [number, number]} col_coord point where cast collided. If null, the sprite is generated only at the trace_crn
   * @param {R2PTraceClass} cls point where cast collided. If null, the sprite is generated only at the trace_crn
   * @returns {Algs.R2PTrace}
   */
  #createTrace(trace_crn, col_coord, cls) {
    const sprite = this.#createTraceSprite(col_coord, trace_crn.coord, cls);
    return new Algs.R2PTrace(trace_crn, sprite);
  }

  /**
   *
   * @param {[number, number] | null} from_coord
   * @param {[number, number]} to_coord
   * @param {R2PTraceClass} cls
   * @returns {Algs.R2PSpriteTrace}
   */
  #createTraceSprite(from_coord, to_coord, cls) {
    const sprite = this.#canvas_trace.getAvailable();
    this.#step.registerWithData(sprite, R2PActionLink.Display, true);
    if (from_coord)
      this.#step.registerWithData(sprite, R2PActionTrace.Add, from_coord);
    this.#setTraceSpriteClass(sprite, cls);
    this.#step.registerWithData(sprite, R2PActionTrace.Add, to_coord);
    return sprite;
  }
   ...
   */
  #removeTraceSprite(sprite) {
    this.#step.registerWithData(sprite, R2PActionTrace.Clear, undefined);
    this.#step.registerWithData(sprite, R2PActionTrace.Display, false);
    this.#canvas_trace.markAvailable(sprite);
  }
   ...
   * @param {R2PTraceClass} cls
   */
  #setTraceSpriteClass(sprite, cls) {
    this.#step.registerWithData(sprite, R2PActionTrace.Class, cls);
  }
};
