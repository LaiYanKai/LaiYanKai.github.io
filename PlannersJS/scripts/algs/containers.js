...
Algs.AbstractPriorityQueue = class {
  ...
  constructor(is_fifo, thresh, ...cost_indices) {
    for (const cost_idx of cost_indices)
      if (!Number.isInteger(cost_idx) || cost_idx < 0)
        throw new Error(
          `cost_indices can only contain non-negative integers. cost_indices="${cost_indices}"`
        );
  ...
  requeue(node) {
    ...
    while (
      queued_node !== null &&
      this.#comparator(node, queued_node) === false
    ) {
  ...
  #insertBefore(new_node, queued_node) {
    if (queued_node === null) {
      ...
      // insert before queued_node
      if (queued_node.pq_prev === null) {
        // queued_node is the front
        queued_node.pq_prev = new_node;
        new_node.pq_next = queued_node;
        new_node.pq_prev = null;
        this.#front = new_node;
      } else {
        // queued_node is somewhere in betweens
  ...
  #compareCosts(new_node, queued_node, tie_breaker) {
    for (const cost_idx of this.#cost_indices) {
      ...
      if (queued_cost > new_node.costs[cost_idx] + this.#thresh) {
        ...
        // queued node has roughly the same cost
  ...
  #remove(node) {
    ...
    if (pq_prev === null)
      // at front
      this.#front = pq_next;
    else pq_prev.pq_next = pq_next;

    if (pq_next === null)
      // at back
      this.#back = pq_prev;
    else pq_next.pq_prev = pq_prev;
