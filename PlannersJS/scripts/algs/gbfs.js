...
Algs.GBFSCanvasCell = class extends UI.AbstractCanvasCell {
    ...
    #setTip(sprite, e) {
        ...
        let status_str;
        if (status === GBFSNodeStatus.Queued)
            status_str = "Queued";
        else if (status === GBFSNodeStatus.Expanding)
            status_str = "Expanding";
        else if (status === GBFSNodeStatus.Visited)
            status_str = "Visited";
        else if (status === GBFSNodeStatus.Undiscovered)
            status_str = "Undiscovered";
        else if (status === GBFSNodeStatus.Path)
            status_str = "Path!";
...
Algs.GBFSNode = class extends Algs.AbstractPriorityQueueNode {
    ...
    get h() { return this.costs[0]; }

    set h(new_h) { return this.costs[0] = new_h; }
...
Algs.GBFS = class extends Algs.AbstractGridAlg {
    ...
    run() {
        ...
        while (this.#openListFilled()) {
            ...
            for (const nb of this.getNb(xpd_node)) {
                ...
                let nb_coord, nb_id, nb_node = null;
                if (nb.in_map === true) {
    ...
    #visualizeExpanded(node) {
        ...
        this.#step.registerWithData(node.sprite, GBFSAction.Status, GBFSNodeStatus.Expanding);
    ...
    #visualizeOpened(node) {
        ...
        this.#step.registerWithData(node.sprite, GBFSAction.Status, GBFSNodeStatus.Queued);
    }

    #visualizeClosed(node) {
        this.#step.registerWithData(node.sprite, SpriteActionNode.Class, SpriteActionClass.Blue);
        this.#step.registerWithData(node.sprite, GBFSAction.Status, GBFSNodeStatus.Visited);
    }
