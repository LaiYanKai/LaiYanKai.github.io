...
Algs.R2PMapper = class {
     ...
     * @returns {null | [Algs.R2PPose, Algs.R2PPose]} returns null if reached, or if collided, an array of two poses.
     */
    cast(from_coord, to_coord) {
        ...
        while (1) {
            let result = this.#castCollided();
            if (result) {
                let [left_pose, right_pose] = result;
        ...
        if (raytracer_state === RayTracerState.X) {
            ...
            if (cell_access === false) {
                ...
                if (sgn[0] > 0) {
                    ...
                    throw new Error(`sgn[0] cannot be zero`);
                if (inner > 0) {
                ...
                } else  // has zero component, can never happen
                    throw new Error(`sgn cannot have zero component`);
                return [left_pose, right_pose];
            }
                ...
                throw new Error(`out of map can never happen in cast collisions in R2+`);
            }
        }
        // ======================= Crossed Horizontal Line ================
            ...
            if (cell_access === false) {
                ...
                if (sgn[1] > 0) {
                    ...
                    throw new Error(`sgn[1] cannot be zero`);
                if (inner > 0) {
                    left_pose.coord = root;
                    right_pose.coord = root_adj;
                } else if (inner < 0) {
        ...
        // ======================= Crossed Vertex ================
            ...
            if (inner === 0) { // cardinal direction;
                ...
                const fl_cell_coord = Utils.adjCellFromVertex(
                    root, Utils.dirIndexToDir(fl_didx));
                const fr_cell_coord = Utils.adjCellFromVertex(
                    root, Utils.dirIndexToDir(fr_didx));
            ...
            // ----------------- Diagonal direction -----------------
                ...
                if (cell_access === false) {
                    ...
                    const left_cell_coord = Utils.adjCellFromVertex(
                        root, Utils.dirIndexToDir(left_didx));
                    const right_cell_coord = Utils.adjCellFromVertex(
                        root, Utils.dirIndexToDir(right_didx));
                    ...
                    else if (left_cell_access !== false && right_cell_access === false) {
                        // hit straight wall
                        left_didx = Utils.addDirIndex(didx, 1);
                        right_didx = Utils.addDirIndex(didx, -3);
                    }
                    else {   // hit convex or non convex corner
                    ...
                    const left_pose = new Algs.R2PPose(
                        R2PSide.L, left_didx, root);
                    const right_pose = new Algs.R2PPose(
                        R2PSide.R, right_didx, root);
                    return [left_pose, right_pose];
                }
                else if (cell_access === true) {
                    return null;
     ...
     * @returns {null | [Algs.R2PPose, Algs.R2PPose]}  Note it is theoretically impossible to return null in R2+ for rectangular maps.
     */
    project(from_coord, diff) {
        let to_coord = from_coord.slice();
        while (1) {
            to_coord = Utils.addCoords(to_coord, diff);
            if (!ui.inMapVertex(to_coord))
                break; // shift the destination to outside the map
        }
