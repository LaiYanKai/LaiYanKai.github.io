...
Algs.R2PPose = class {
     ...
     */
    constructor(side = R2PSide.L, didx = DirIndex.NW, coord = [0, 0], convex = false) {
        this.side = side;
        this.didx = didx;
        this.coord = coord;
        this.convex = convex;
...
Algs.R2PCorner = class extends Algs.R2PPose {
    ...
    best(tdir) {
        if (tdir === R2PTreeDir.Src) {
            if (!this.#best_src)
                this.#best_src = new Algs.R2PBest();
            return this.#best_src;
        }
        else if (tdir === R2PTreeDir.Tgt) {
            if (!this.#best_tgt)
                this.#best_tgt = new Algs.R2PBest();
    ...
    registerLink(link, ldir = R2PListDir.Auto) {
        if (link.crn !== this)
            throw (`adding link to wrong corner`);
        ...
        else if (ldir === R2PListDir.Front)
            links.unshift(link);
        else // R2PListDir.Back
            links.push(link);
    }
    ...
    unregisterLink(link) {
        if (link.crn !== this)
            throw (`removing link from wrong corner`);
        ...
        if (idx >= 0)
            ...
            throw new Error(`not found link ${link}`);
    }
...
Algs.R2PCorners = class {
     ...
     * @returns {number} 
     */
    #serializer(side, didx, coord) { return ui.serializeVertex(coord) * 16 + Utils.wrapDirIndex(didx) * 2 + (side === R2PSide.R); }
    ...
    */
    at(side, didx, coord, convex) {
        ...
        if (this.#corners.has(id))
            return this.#corners.get(id);
        else {
            const crn = new Algs.R2PCorner(
                side, didx, coord, convex, id);
...
Algs.R2PLink = class {
    ...
    toString(type = 1) {
        let str = `[L${this.id}:${R2PTreeDir.toString(this.tdir)}${R2PLinkType.toString(this.type)}]`;
        if (this.crn)
            str = str.concat(`(${this.crn.toString()})`);
        else
            str = str.concat("(null)");
        ...
        if (type >= 1) {
            if (this.prog_ray)
                str = str.concat(`, Prg(${this.prog_ray.join(",")})`);
            else
                str = str.concat(`, Prg(null)`);
            if (this.left_ray)
                str = str.concat(`, LRay(${this.left_ray})`);
            else
                str = str.concat(`, LRay(null)`);
            if (this.right_ray)
                str = str.concat(`, RRay(${this.right_ray})`);
            else
                str = str.concat(`, RRay(null)`);
        }

        if (type >= 1) {
            str = str.concat(`, srcs[${this.#srcs.length}] {`);
            for (const src of this.#srcs)
                str = str.concat(`${src.toString(0)}, `);
            str = str.concat(`}, tgts [${this.#tgts.length}] {`);
            for (const tgt of this.#tgts)
                str = str.concat(`${tgt.toString(0)}, `);
            str = str.concat(`}`);
        }
     ...
     * @returns {undefined | Algs.R2PLink}
     */
    getFirstRootLink() { return this.#links(this.tdir)[0]; }

    /** 
     * Gets the corner at the root (opposite of anchor)
     * @returns {Algs.R2PCorner}
     */
    getRootCrn() { return this.getFirstRootLink().crn; }

    /**
     * Returns the difference vector of the anchor's coordinates from the root coordinates.
     * @returns {[number, number]}
     */
    getDiff() { return Utils.subtractCoords(this.crn.coord, this.getRootCrn().coord); }

    getLength() { return Utils.euclidean(this.getDiff()); }
    ...
    getMinRootNeighborCost() {
        ...
        for (const nb_link of nb_links) {
            if (nb_link.cost < min_cost)
                min_cost = nb_link.cost;
        }
        return min_cost;
    }
    ...
    /** @returns {Algs.R2PLink[]} */
    #links(tdir) { return tdir === R2PTreeDir.Src ? this.#srcs : this.#tgts; }
     ...
     * @returns {number} 
     */
    numNeighbors(tdir) { return this.#links(tdir).length; }
     ...
     */
    addNeighbor(tdir, new_link) { this.#links(tdir).push(new_link); }
     ...
     * @returns {undefined | Algs.R2PLink}
     */
    neighborAt(tdir, idx) { return this.#links(tdir).at(idx); }
...
Algs.R2PLinks = class {
    ...
    createLink() {
        let new_link;
        if (this.#unused_links.length > 0)
            new_link = this.#unused_links.pop();
        else {
...
Algs.R2POpenListNode = class {
    ...
    toString() {
        return `${R2PQueueType.toString(this.type)}\$${this.f.toFixed(3)}{ ${this.link.toString(0)} }`;
    }
};
...
Algs.R2POpenList = class {
    ...
    queue(type, link, f) {
        if (link.qnode)
            throw new Error(`Queued > once ${link}!`);
        ...
        if (prev_qnode)
            prev_qnode.next = new_qnode;
        else
            this.#front = new_qnode;
        new_qnode.prev = prev_qnode;
        if (qnode)
            qnode.prev = new_qnode;
    ...
    unqueue(link) {
        ...
        if (prev_qnode)
            prev_qnode.next = next_qnode;
        else
            this.#front = next_qnode;
        if (next_qnode)
            next_qnode.prev = prev_qnode;
        qnode.prev = null;
        qnode.next = null;
        return qnode;
    }
...
Algs.R2PRay = class {
    ...
    getCoord() { return [this.dx, this.dy]; }

    copy() { return new Algs.R2PRay(this.dx, this.dy, this.closed, this.from_src); }

    toString() { return `${this.dx},${this.dy},${(this.closed ? "1" : "0")}`; }
};