...
Algs.GraphNode = class extends Algs.AbstractPriorityQueueNode {
  //randomSamplingNode
  ...
  constructor (
    ...
    super(value_XY, 1, sprite, null);
    this.parent = parent;
    this.value_XY = value_XY; //rename
    this.neighbors = neighbors;
    this.g = g_cost; // only cost
    this.sprite = sprite;
  }
};
...
Algs.AbstractAlg = class {
   ...
   */
  *canvases() {
    for (const canvas of this.#canvases) yield canvas;
  }

  /** @returns {UI.AbstractCanvas} */
  canvas(idx) {
    return this.#canvases[idx];
  }
   ...
   */
  *lenses() {
    for (const lens of this.#lenses) yield lens;
  }

  /** @returns {UI.AbstractLens} */
  lens(idx) {
    return this.#lenses[idx];
  }
  ...
  setLenses(lenses, default_lens_idx) {
    ...
    if (
      !Utils.isFiniteNonNegativeInteger(default_lens_idx) ||
      default_lens_idx > lenses.length
    )
      throw new RangeError(
        `"default_lens_idx" must be >= 0 and shorter than the length of "lenses"`
      );
    let num_lens_none = 0;
    for (const lens of lenses) {
      if (lens instanceof UI.AbstractLens === false)
        throw new TypeError(`"lenses" must be an array of UI.AbstractLens`);
      if (lens instanceof UI.LensNone) ++num_lens_none;
    }
    if (num_lens_none !== 1) {
  ...
  #setRanks(rank_names, default_rank) {
    ...
    if (
      !Utils.isFiniteNonNegativeInteger(default_rank) ||
      default_rank >= rank_names.length
    )
      throw new RangeError(
        `"default_rank" must be between 0 and 1 minus the length of "rank_names" inclusive.`
      );
...
Algs.GridAlgNeighbor = class {
  ...
  constructor (didx, metric_function) {
    ...
    this.metric_length =
      metric_function == null ? 1 : metric_function(this.dir);
...
Algs.AbstractGridAlg = class extends Algs.AbstractAlg {
  ...
  constructor (rank_names, default_rank, params) {
    ...
    // Initalize InMap
    if (this.params.node_type === AlgNodeType.Cell) this.inMap = ui.inMapCell;
    else if (this.params.node_type === AlgNodeType.Vertex)
    ...
    else if (this.params.checkerboard === null) this.#checkerboard = null;
    ...
    else if (this.params.distance_metric === null) this.metric = null;

    // Initialize this.#dind_order
    this.#dind_order = [];
    if (this.params.node_connectivity === AlgNodeConnectivity.EightConnected)
      this.#dind_order = [
        ...Utils.dirIndexRange(this.params.first_neighbor, 1),
      ];
    else if (
      this.params.node_connectivity === AlgNodeConnectivity.FourConnected
    )
      this.#dind_order = [
        ...Utils.dirIndexRange(this.params.first_neighbor, 2),
      ];
    ...
    else if (this.params.g_nb === AlgGNb.Min) this.#gNb = this.#gMin.bind(this);
    else if (this.params.g_nb === AlgGNb.Max) this.#gNb = this.#gMax.bind(this);
    else if (this.params.g_nb === null) this.#gNb = this.#gUnity.bind(this);

    this.#is_binary = this.params.costmap_type === AlgCostmapType.Binary;

    const has_lethal =
      this.params.costmap_type === AlgCostmapType.Binary ||
    ...
    } else this.#nbCellInfo = this.#nbCellInfoN.bind(this);
    if (this.params.node_type === AlgNodeType.Cell)
    ...
    // assign #nbNodeInfos
    if (this.params.node_type === AlgNodeType.Cell) {
      this.#nbNodeInfoC = has_lethal
        ? this.#nbNodeInfoCL.bind(this)
        : this.#nbNodeInfoCN.bind(this);
      this.#nbNodeInfos = this.#nbNodeInfosC.bind(this);
    } else if (this.params.node_type === AlgNodeType.Vertex) {
      this.#nbNodeInfoV = has_lethal
        ? this.#nbNodeInfoVL.bind(this)
        : this.#nbNodeInfoVN.bind(this);
  ...
  #setStart(coord_start) {
    ...
    if (!this.inMap(coord_start))
      throw new RangeError(
        `coord_start is not in the map. coord_start="${coord_start}"`
      );
  ...
  #setGoal(coord_goal) {
    ...
    if (!this.inMap(coord_goal))
      throw new RangeError(
        `coord_goal is not in the map. coord_goal="${coord_goal}"`
      );
  ...
  #nbCellInfosV(exp_node) {
    for (const neighbor of this.#neighbors) {
      const neighbor_vertex_coord = Utils.addCoords(
        exp_node.coord,
        neighbor.dir
      );
      neighbor.in_map = this.inMap(neighbor_vertex_coord);
      if (!neighbor.is_cardinal) {
        const neighbor_cell_coord = Utils.addCoords(
          exp_node.coord,
          neighbor.adj_cell_dir
        );
        this.#nbCellInfo(neighbor_cell_coord, neighbor);
      }
    }
  ...
  #nbNodeInfoCL(neighbor, exp_node) {
    ...
    if (this.#checkerboard === true) {
      if (neighbor.is_cardinal === false) {
        ...
        neighbor.node_chblocked =
          nb_fl.cell_access === false && nb_fr.cell_access === false;
      } else neighbor.node_chblocked = false;
      ...
      if (neighbor.is_cardinal) {
        ...
        if (neighbor.node_access === true) {
          ...
          if (this.#checkerboard === true) {
            ...
            if (exp_node.parent === null) {
              // start node, allow checkerboard access.
              neighbor.node_chblocked = false;
            } else {
              // check for checkerboard acceess
              const parent_dir = Utils.subtractCoords(
                exp_node.parent.coord,
                exp_node.coord
              );
              ...
              neighbor.node_chblocked =
                (!nb_fl.cell_access &&
                  !nb_br.cell_access &&
                  Utils.dotCoords(parent_dir, nb_fr.dir) < 0) ||
                (!nb_bl.cell_access &&
                  !nb_fr.cell_access &&
                  Utils.dotCoords(parent_dir, nb_fl.dir) < 0);

              neighbor.node_access = !neighbor.node_chblocked;
              if (!neighbor.node_access) {
                // if no checkerboard access, no need to calculate cost
                return;
              }
            }
          } // if checkerboard
        ...
        if (neighbor.node_access === true) {
          ...
          if (this.#checkerboard === true) {
            ...
            if (exp_node.parent === null) {
              // start node has no parent, allow checkerboard access
              neighbor.node_chblocked = false;
            } else {
              // not a start node, check for checkerboard access
              const parent_dir = Utils.subtractCoords(
                exp_node.parent.coord,
                exp_node.coord
              );
              ...
              neighbor.node_chblocked =
                !nb_l.cell_access &&
                !nb_r.cell_access &&
                Utils.dotCoords(parent_dir, neighbor.dir) < 0;
  ...
  getNb = function* (exp_node) {
    ...
    for (const didx of this.#dind_order) yield this.#neighbors[didx];
  };

  /**
   * Returns the threshold cost for the cells to be considered as lethal.
   * @returns {number} the threshold cost */
  getLethal() {
    if (this.#lethal === undefined) return Infinity;
    return this.#lethal;
  }

  /**
   * Returns the cost type
   * @returns {boolean} if the alg has a binary cost */
  isBinary() {
    return this.#is_binary;
  }

  /**
   * Returns the checkerboard status
   * @returns {boolean} if the alg has checkerboard blocking */
  hasCheckerboard() {
  ...
  tracify(coord) {
    if (this.params.node_type === AlgNodeType.Cell)
      return Utils.addCoords(coord, [0.5, 0.5]);
    else if (this.params.node_type === AlgNodeType.Vertex)
      return coord;
    console.warn("Unknown coordinate type");
  }
};
