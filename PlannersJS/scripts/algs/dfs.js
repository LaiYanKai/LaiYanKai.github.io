...
Algs.DFSCanvasCell = class extends UI.AbstractCanvasCell {
    ...
    #setTip(sprite, e) {
        ...
        let status_str;
        if (status === DFSNodeStatus.Queued)
            status_str = "Queued";
        else if (status === DFSNodeStatus.Expanding)
            status_str = "Expanding";
        else if (status === DFSNodeStatus.Visited)
            status_str = "Visited";
        else if (status === DFSNodeStatus.Undiscovered)
            status_str = "Undiscovered";
        else if (status === DFSNodeStatus.Path)
            status_str = "Path!";
...
Algs.DFS = class extends Algs.AbstractGridAlg {
    ...
    #StackNode(node) {
        if (this.#stack.insert(node) === -1) {    // visited
        }
        else {   // newly queued
    ...
    #visualizeExpanded(node) {
        ...
        this.#step.registerWithData(node.sprite, DFSAction.Status, DFSNodeStatus.Expanding);
    ...
    #visualizeOpened(node) {
        ...
        this.#step.registerWithData(node.sprite, DFSAction.Status, DFSNodeStatus.Queued);
    }

    #visualizeClosed(node) {
        this.#step.registerWithData(node.sprite, SpriteActionNode.Class, SpriteActionClass.Blue);
        this.#step.registerWithData(node.sprite, DFSAction.Status, DFSNodeStatus.Visited);
    }
