...
Algs.R2PCanvasLinks = class extends UI.AbstractCanvas {
    ...
    constructor(canvas_id) {
        ...
        this.#sprite_sector = new Algs.R2PSpriteSector(canvas_id, `r2s_${canvas_id}`);
    ...
    add(id,
        // src_coord = null, tgt_coord = null, cost = Infinity,
        // side = R2PSide.L, tdir = R2PTreeDir.Src, type = R2PLinkType.Vu,
        // left_ray = [null, false], right_ray = [null, false],
        // prog_ray = null,
        // src_ids = [], tgt_ids = [], status = R2PLinkStatus.Placing,
        // knob = 0, display = false, z_index = 0
    ) {
        /** @type {Algs.R2PSpriteLink} */
        const sprite = super.add(id);
        // sprite.register(R2PActionLink.Cost, cost);
        ...
        // sprite.register(R2PActionLink.SrcCoord, src_coord);
        // sprite.register(R2PActionLink.Tdir, tdir);
        // sprite.register(R2PActionLink.TgtCoord, tgt_coord);
        // sprite.register(R2PActionLink.Type, type);

        // for (const id of src_ids)
        //     sprite.register(R2PActionLink.AddSrcId, [id, true]);
        // for (const id of tgt_ids)
        //     sprite.register(R2PActionLink.AddTgtId, [id, true]);
        ...
        sprite.dom.addEventListener(
            "mousemove", this.#mouseMove.bind(this, sprite), false);
        sprite.dom.addEventListener(
            "mouseout", this.#mouseOut.bind(this, sprite), false);
    ...
    #setSectorAndProg(sprite, e) {
        ...
        if (prog_ray) {
            ...
            const from_coord = (tdir === R2PTreeDir.Src) ? 
                src_coord : 
        ...
        // const link_status = sprite.value(R2PActionLink.Status);
        // if (link_status !== R2PLinkStatus.Expanding)
        this.#sprite_sector.draw(true, 0,
            src_coord, left_ray, right_ray, R2PSectorClass.Inactive);
    }
    ...
    #setTip(sprite, e) {
        ...
        if (type === R2PLinkType.Eu)
            type_str = "Eu"; //Anchor is <u>E</u>xpensive point with <u>u</u>nknown cumulative visibility";
        else if (type === R2PLinkType.Ey)
            type_str = "Ey"; //Anchor is <u>E</u>xpensive point with cumulative visibility (<u>y</u>es)";
        else if (type === R2PLinkType.Oc)
            type_str = "Oc"; // Anchor from limited trace in target <u>oc</u>cupied sector";
            ...
            type_str = "Vu"; // Anchor has <u>u</u>nknown cumulative <u>v</u>isibility";
        else if (type === R2PLinkType.Vy)
            type_str = "Vy"; // Anchor has cumulative <u>v</u>isibility (<u>y</u>es)";
        type_str = `${type_str}`;
        ...
        else if (status === R2PLinkStatus.Holding)
            status_str = "Holding";
        else if (status === R2PLinkStatus.Pruning)
            status_str = "Pruning";
        else if (status === R2PLinkStatus.Placing)
            status_str = "Placing";
        else if (status === R2PLinkStatus.Queued)
            status_str = "Queued";
        else if (status === R2PLinkStatus.OutOfAngularSector)
            status_str = "Out of Angular Sector!";
        else if (status === R2PLinkStatus.OutOfOccupiedSector)
            status_str = "Out of Occupied Sector!";
        else if (status === R2PLinkStatus.Path)
            status_str = "Path!";

        // ================= Rays ========================
        const left_ray = sprite.value(R2PActionLink.LeftRay);
        const right_ray = sprite.value(R2PActionLink.RightRay);
        let left_ray_str = "None", right_ray_str = "None";
        if (left_ray)
            left_ray_str = `[${left_ray.dx}, ${left_ray.dy}], ${(left_ray.closed ? "closed" : "open")}`;
        if (right_ray)
            right_ray_str = `[${right_ray.dx}, ${right_ray.dy}], ${(right_ray.closed ? "closed" : "open")}`;
        const prog_ray = sprite.value(R2PActionLink.ProgRay);
        let prog_ray_str = ``;
        if (prog_ray)
            prog_ray_str = `[${prog_ray.join(",")}]`;
