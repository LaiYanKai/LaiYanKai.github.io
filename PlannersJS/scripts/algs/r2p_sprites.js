...
Algs.R2PSpriteLink = class extends UI.AbstractSprite {
    ...
    getSrcIds() { return this.#src_ids.slice(); }
    getTgtIds() { return this.#tgt_ids.slice(); }
    ...
    #tryAddRels(redo, action_id, data) {
        const ids = action_id === R2PActionLink.AddSrcId ? this.#src_ids : this.#tgt_ids;
        let [id, add] = data;
        ...
        if (add && idx < 0)
            ...
            return false; // no action should be registered.
        return true;
    }
    ...
    #setClass(tdir, status, type) {
        const clsList = this.dom.classList;
        ...
        if (status === R2PLinkStatus.Expanding)
            clsList.add("exp");
        else clsList.remove("exp");
        if (status === R2PLinkStatus.Holding)
            clsList.add("hol");
        else clsList.remove("hol");
        if (status === R2PLinkStatus.Pruning)
            clsList.add("pru");
        else clsList.remove("prn");
        if (status === R2PLinkStatus.Queued)
            clsList.add("que");
        else clsList.remove("que");
        if (status === R2PLinkStatus.Path)
            clsList.add("path");
        else clsList.remove("path");

        if (type === R2PLinkType.Ey)
            clsList.add("ey");
        else clsList.remove("ey");
        if (type === R2PLinkType.Vy)
            clsList.add("vy");
        else clsList.remove("vy");
    }
     ...
     */
    #drawCurveAndArrow(min_bound, max_bound,
        side, knob, anchor_coord, root_coord,
        anchor_radius, root_radius, param_knob,
        param_arrow_length, param_arrow_width, param_arrow_position
    ) {
        ...
        if (diff[0] === 0 && diff[1] === 0) {
            ...
            let knob_coord = Utils.divideCoord(Utils.addCoords(anchor_coord, root_coord), 2); // halfway point
            knob_coord = Utils.addCoords(knob_coord,
            ...
            this.#dom_curve.setAttribute("d",
                `M ${curve_anchor_coord.join(",")} Q ${knob_coord.join(",")} ${curve_root_coord.join(",")}`);

            // ============= Derive arrow coordinates ==================
            let arrow_tip_coord = [
                Utils.quadraticBezier(param_arrow_position, curve_anchor_coord[0], knob_coord[0], curve_root_coord[0]),
                Utils.quadraticBezier(param_arrow_position, curve_anchor_coord[1], knob_coord[1], curve_root_coord[1]),
            ];
            let unit_arrow = Utils.unitCoord([
                Utils.quadraticBezierGrad(param_arrow_position, curve_anchor_coord[0], knob_coord[0], curve_root_coord[0]),
                Utils.quadraticBezierGrad(param_arrow_position, curve_anchor_coord[1], knob_coord[1], curve_root_coord[1]),
            ]);
            let unit_normal = [unit_arrow[1], -unit_arrow[0]];
            unit_arrow = Utils.addCoords(arrow_tip_coord, // reuse as position at the other end of the arrow.
                Utils.multiplyCoord(unit_arrow, -param_arrow_length));
            let arrow1 = Utils.addCoords(unit_arrow,
                Utils.multiplyCoord(unit_normal, param_arrow_width / 2));
            let arrow2 = Utils.addCoords(unit_arrow,
                Utils.multiplyCoord(unit_normal, -param_arrow_width / 2));

            // ============= Draw the arrow ===============
            this.#dom_arrow.setAttribute("d",
                `M ${arrow_tip_coord.join(",")} L ${arrow1.join(",")} L ${arrow2.join(",")} Z`);
     ...
     * @param {number} anchor_radius 
     */
    #drawAnchor(min_bound, max_bound,
        type, anchor_coord, anchor_radius) {
        ...
        if (type === R2PLinkType.Eu || type === R2PLinkType.Ey) {
            ...
            this.#dom_anchor.setAttribute("d",
                `M ${coord0.join(",")} L ${coord1.join(",")} L ${coord2.join(",")} L ${coord3.join(",")} Z`);
            if (type === R2PLinkType.Ey)
                this.#dom_anchor.classList.add("fill");
            else
            ...
            const coord0 = Utils.addCoords(anchor_coord, [anchor_radius, anchor_radius]);
            const coord1 = Utils.addCoords(anchor_coord, [-anchor_radius, anchor_radius]);
            const coord2 = Utils.addCoords(anchor_coord, [-anchor_radius, -anchor_radius]);
            const coord3 = Utils.addCoords(anchor_coord, [anchor_radius, -anchor_radius]);
            this.#dom_anchor.setAttribute("d",
                `M ${coord0.join(",")} L ${coord1.join(",")} L ${coord2.join(",")} L ${coord3.join(",")} Z`);
            this.#dom_anchor.classList.remove("fill");
            Utils.updateMinBound(min_bound,
            ...
            this.#dom_anchor_line.setAttribute("d",
            ...
            const half1 = `A ${anchor_radius} ${anchor_radius} 0 0 1 ${coord0.join(",")}`;
            this.#dom_anchor.setAttribute("d", `M ${coord0.join(",")} ${half0} ${half1}`);
            this.#dom_anchor.classList.remove("fill");
            Utils.updateMinBound(min_bound,
                Utils.addCoords(anchor_coord, [-anchor_radius, -anchor_radius]));
            Utils.updateMaxBound(max_bound,
            ...
            this.#dom_anchor_line2.setAttribute("d",
            ...
            throw new RangeError(`type should be a member of R2PLinkType:${R2PLinkType}`);
        }
    }

    #setDimensions(min_bound, max_bound,
        param_stroke_width
    ) {
        ...
        this.dom.setAttribute("viewBox", `${min_bound[0]} ${min_bound[1]} ${size[0]} ${size[1]}`);
        this.dom.style.inset = `${min_bound[1]}px auto auto ${min_bound[0]}px`;
    }


    register(action_id, new_data) {
        if (action_id === R2PActionLink.AddSrcId || action_id === R2PActionLink.AddTgtId) {
            if (!this.#tryAddRels(true, action_id, new_data))
                return; // no register if an id is already found in the src_ids, tgt_ids.}
        }
        super.register(action_id, new_data);
    }
    ...
    vis() {
        // check display
        if (!this.#setDisplay(this.value(R2PActionLink.Display)))
            return;

        //  ============= Get values =====================
        const root_radius = ui_params.sprite_link_root_radius * ui_params.cell_size;
        const anchor_radius = ui_params.sprite_link_anchor_radius * ui_params.cell_size;
        ...
        // ============== Curve and Arrow ==================
        this.#drawCurveAndArrow(min_bound, max_bound,
            side, knob, anchor_coord, root_coord,
            anchor_radius, root_radius, ui_params.sprite_link_knob,
            ui_params.sprite_link_arrow_length,
            ui_params.sprite_link_arrow_width,
            ui_params.sprite_link_arrow_position);

        // ============== anchor ==================
        this.#drawAnchor(min_bound, max_bound,
            type, anchor_coord, anchor_radius);

        //=============== Set dimensions ==================
        this.#setDimensions(min_bound, max_bound,
...
Algs.R2PSpriteSector = class extends UI.AbstractSprite {
    ...
    #setClass(cls) {
        let add_cls = [], rem_cls = [];
        if (cls === R2PSectorClass.Active)
            add_cls.push("active");
        else
            rem_cls.push("active");
        if (cls === R2PSectorClass.Inactive)
            add_cls.push("inactive");
        else
            rem_cls.push("inactive");
        for (const cls of add_cls)
            this.dom.classList.add(cls);
        for (const cls of rem_cls)
            this.dom.classList.remove(cls);
    }

    #drawSrc(min_bound, max_bound,
        src_coord, src_radius) {
    ...
    #drawRayAndExt(
        min_bound, max_bound,
        side, src_coord, src_radius, ray_radius,
        ray_diff, from_src,
        ray_arrow_length, ray_arrow_width, map_px_size) {
        ...
        // ============== Draw the ray ======================
        dom_ray.setAttribute("d",
            `M ${ray_from_coord.join(",")} L ${ray_to_coord.join(",")} L ${arrow0.join(",")} L ${arrow1.join(",")} L ${arrow2.join(",")} L ${arrow3.join(",")}`);

        // =============== Determine ext coordinates ===========
        let ext_from_coord;
        if (from_src)
            ext_from_coord = ray_to_coord;
        else
            ext_from_coord = [
                src_coord[0] + ray_unit_diff[0] * src_radius,
                src_coord[1] + ray_unit_diff[1] * src_radius,
    ...
    #drawSector(min_bound, max_bound,
        src_radius, sector_radius,
        left_ray_diff, right_ray_diff, src_coord) {
        ...
        for (let a of [-1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5]) {
            ...
            if (Utils.approxGt(angle, angle_left) &&
        ...
        const pattern_topleft = Utils.subtractCoords(src_coord, [ext_radius, ext_radius])
        this.#dom_pattern.setAttribute("width", sector_radius * 2);
        this.#dom_pattern.setAttribute("height", sector_radius * 2);
        this.#dom_pattern.setAttribute("patternTransform",
    ...
    draw(display, z_index, src_coord, left_ray, right_ray, cls) {
        ...
        // ============== Src =================
        this.#drawSrc(min_bound, max_bound,
            src_coord, src_radius);

        // ============== Draw Rays ==================
        this.#drawRayAndExt(min_bound, max_bound,
            R2PSide.L, src_coord, src_radius, ray_radius,
            left_ray_diff, left_from_src,
            param_ray_arrow_length, param_ray_arrow_width, map_px_size);
        this.#drawRayAndExt(min_bound, max_bound,
            R2PSide.R, src_coord, src_radius, ray_radius,
            right_ray_diff, right_from_src,
            param_ray_arrow_length, param_ray_arrow_width, map_px_size);

        // ============== Draw Sector ========================
        this.#drawSector(min_bound, max_bound,
            src_radius, sector_radius,
            left_ray_diff, right_ray_diff, src_coord);

        //=============== Set dimensions ==================
        this.#setDimensions(min_bound, max_bound,
            ui_params.sprite_sec_ray_width); // assumes sprite_sec_ext_width is smaller.
    }
...
Algs.R2PSpriteProg = class extends UI.AbstractSprite {
    ...
    draw(display, z_index, src_coord, diff) {
        ...
        this.dom.setAttribute("viewBox", `${-transform_origin[0]} ${-transform_origin[1]} ${width} ${height}`);

        // ============ Rotate ==================
        this.dom.style.transformOrigin =
            `${transform_origin[0]}px ${transform_origin[1]}px`;
...
Algs.R2PSpriteTrace = class extends UI.AbstractSprite {
    ...
    #setDimensions(min_bound, max_bound,
        ...
        this.dom.setAttribute("width", `${size[0]}px`);
        this.dom.setAttribute("height", `${size[1]}px`);
        this.dom.setAttribute("viewBox", `${min_bound.join(" ")} ${size.join(" ")}`);
        this.dom.style.inset = `${min_bound[1]}px auto auto ${min_bound[0]}px`;
    }
    ...
    draw(display, z_index, cls, coords) {
        ...
        for (const coord of coords) {
            ...
            if (path_str === "")
                ...
                path_str = `${path_str} L${px.join(",")}`;
            Utils.updateBounds(min_bound, max_bound, px);
        }
        dom_path.setAttribute("d", path_str);

        // ============= viewBox =========================
        this.#setDimensions(min_bound, max_bound, ui_params.sprite_trace_stroke_width);
    }

    register(action_id, new_data) {
        if (action_id === R2PActionTrace.Add)
            this.#stack.push(new_data);
        super.register(action_id, new_data);
    }

    redo(action_id) {
        const redone = super.redo(action_id);
        if (redone && action_id === R2PActionTrace.Add)
            this.#stack.push(new_data);
        return redone;
    }

    undo(action_id) {
        if (action_id === R2PActionTrace.Add && !this.atFirstStep(action_id))
            this.#stack.pop();
        return super.undo(action_id);
    }
