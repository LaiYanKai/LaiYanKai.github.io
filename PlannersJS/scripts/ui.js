...
const ui_params = {
  ...
  /** @type {number} */
  sprite_anya_stroke_width: undefined,
  /** @type {number} */
  sprite_anya_ray_arrow_length: undefined,
  /** @type {number} */
  sprite_anya_ray_arrow_width: undefined,
  /** @type {number} */
  sprite_anya_root_radius: undefined,
  /** @type {number} */
  sprite_rrt_circ_stroke_width: undefined,
  /** @type {number} */
  play_interval: 20,
  /** @type {number} */
  thresh: 1e-8,
  ...
  init() {
    ...
    this.edge_width = parseFloat(getCssVar("--edge-width"));
    this.line_cap = parseFloat(getCssVar("--edge-cap"));
    ...
    this.sprite_anya_stroke_width = parseFloat(getCssVar("--sprite-anya-stroke-width"));
    this.sprite_anya_ray_arrow_length = parseFloat(getCssVar("--sprite-anya-ray-arrow-length"));
    this.sprite_anya_ray_arrow_width = parseFloat(getCssVar("--sprite-anya-ray-arrow-width"));
    this.sprite_anya_root_radius = parseFloat(getCssVar("--sprite-anya-root-radius"));

    this.sprite_rrt_circ_stroke_width = parseFloat(getCssVar("--sprite-rrt-circ-stroke-width"));
...
const ui = {
  ...
  init() {
    ...
    this.dynamicFormSelectionMethod(); // to deprecate, put in form_algs.

    this.drawMode();
    this.makeMap(ui_states, Utils.createArray2d(ui_states.size, 1), false);
    ...
    ui.form_alg_algorithm.selectValue(AlgAlgorithm.AStar);
  ...
  dynamicFormSelectionMethod() { // note this should go under form_algs
    // enables dynamic showing of UI forms


    document
      .getElementById("form_alg_neighbor_selection_method")
      .addEventListener("change", function () {
        if (this.value == 1) {
          ui[`form_alg_neighbor_selection_method_knn_number`].hide();

          ui[`form_alg_neighbor_selection_method_radius_number`].show();
        } else if (this.value == 0) {
          ui[`form_alg_neighbor_selection_method_radius_number`].hide();

          ui[`form_alg_neighbor_selection_method_knn_number`].show();
        }
      });


    document
      .getElementById("form_alg_nearby_nodes_selection_method")
      .addEventListener("change", function () {
        if (this.value == 1) {
          ui[`form_alg_nearby_nodes_selection_method_nearest_number`].hide();

          ui[`form_alg_nearby_nodes_selection_method_radius_number`].show();
        } else if (this.value == 0) {
          ui[`form_alg_nearby_nodes_selection_method_radius_number`].hide();

          ui[`form_alg_nearby_nodes_selection_method_nearest_number`].show();
        }
      });
  ...
  testTra() {
    ...
    // ui.form_new_map_source.selectValue(NewMapSource.Template);
    // ui.form_new_map._change_source();
    // ui.form_new_map_template.selectValue(NewMapTemplate.Maze02);
    // ui.dialog.ok();
    // ui.newMap();
    // ui.form_new_map_source.selectValue(NewMapSource.Template);
    // ui.form_new_map._change_source();
    // ui.form_new_map_template.selectValue(NewMapTemplate.Test03); // NewMapTemplate.Test01, NewMapTemplate.Test02
    // ui.dialog.ok();

    ui.showDialog(ui.form_alg);
    ui.form_alg_algorithm.selectValue(AlgAlgorithm.RRT);
    ui.form_alg._change_algorithm();
    ui.dialog.ok();
  ...
  testYK() {
    ...
    for (const coord of [
      ...
      let cell = ui.cells.at(coord);
      cell.cost = 10;
      cell.vis();
    }

    const rt = new RayTracerGeneral();
    console.log(
      rt.run([7, 3], [2, 3], false, (coord) => {
        const cell = ui.cells.at(coord);
        return cell === null || cell.cost >= 5;
      })
    );
    console.log(rt.getRoot());

    let canvas = new Algs.AnyaCanvasNodes(0);
    let canvas_arr = new UI.AbstractCanvasArrow(1, 0);
    ui.layers.loadCanvases([canvas, canvas_arr]); // Use Alg.setCanvas. See A* constructor.

    const sprite0 = canvas.addCone(
      0,
      true,
      0,
      [4, 8],
      AnyaNodeStatus.Queued,
      Infinity,
      9.8,
      Infinity,
      [5, -6],
    ...
    const sprite1 = canvas.addFlat(
      1,
      true,
      0,
      [8, 8],
      AnyaNodeStatus.Expanding,
      Infinity,
      9.8,
      Infinity,
      [2, 0]
    );
    const spritea = canvas_arr.add(
      0,
      [8, 8],
      [-4, 0],
      true,
    ...
    //rrt
    const sprite_r = new Algs.RRTSpriteCircle(0);
    canvas.dom.appendChild(sprite_r.dom);
    sprite_r.register(RRTActionCircle.Display, true);
    sprite_r.register(RRTActionCircle.ZIndex, 0);
    sprite_r.register(RRTActionCircle.Position, [10.1, 13.2]);
    sprite_r.register(RRTActionCircle.Radius, 5.42);
    sprite_r.vis();
  },

  get printConfig() {
    let arr = [];
    for (let i = 0; i < ui_states.size[0]; i++) {
      let row = [];
      for (let j = 0; j < ui_states.size[1]; j++) {
        row.push(ui.cells.at([i, j]).cost);
      }
      arr.push(row);
    }

    const states = `{ "dialog": false, "draw_mode": true, "draw_cost": 10, "visual_min_cost": 0, "visual_max_cost": 10, "draw_step": 1, "goal": ${JSON.stringify(ui_states.alg.coord_goal)}, "start": ${JSON.stringify(ui_states.alg.coord_start)}, "size": [30, 20], "rank": 0, "changing_start": false, "changing_goal": false, "changing_draw_cost": false, "changing_rank": false, "changing_lens": false, "alg": null }`;
    console.log(`{ "costs": ${JSON.stringify(arr)}, "states": ${states} }`);
  },
  ...
  testTZ() {
    ...
    const euclFromGoal = coord => Utils.euclidean(Utils.subtractCoords(coord, [0, 0]));

    let node, successors;

    // console.log("new node:");
    // node = new Algs.AnyaConeNode([1, 1], 0, 0, 0, 0, new Algs.AnyaInterval([2, 3], [4, 3],
    //     AnyaIntervalType.Closed), isOccupied, euclFromGoal, false);
    // console.log(node.toString());
    // successors = node.generateSuccessors();
    // console.log("All successors:");
    // successors.forEach(s => console.log(s.toString()));

    // console.log("new node:");
    // node = new Algs.AnyaFlatNode([0, 9], 0, 0, 0, 0, new Algs.AnyaInterval([0, 9], [2, 9],
    //     AnyaIntervalType.Closed), isOccupied, euclFromGoal, false);
    // console.log(node.toString());
    // successors = node.generateSuccessors();
    // console.log("All successors:");
    ...
    // console.log("new node:");
    // node = new Algs.AnyaConeNode([1, 11], 0, 0, 0, 0, new Algs.AnyaInterval([1, 13], [3, 13],
    //     AnyaIntervalType.Closed), isOccupied, euclFromGoal, false);
    // console.log(node.toString());
    // successors = node.generateSuccessors();
    // console.log("All successors:");
    ...
    // console.log("new node:");
    // node = new Algs.AnyaFlatNode([1, 19], 0, 0, 0, 0, new Algs.AnyaInterval([1, 19], [3, 19],
    //     AnyaIntervalType.Closed), isOccupied, euclFromGoal, false);
    // console.log(node.toString());
    // successors = node.generateSuccessors();
    // console.log("All successors:");
    ...
    ui.showDialog(ui.form_alg);
    ui.form_alg_algorithm.selectValue(AlgAlgorithm.Anya);
    ui.form_alg._change_algorithm();
    ui.dialog.ok();
  ...
  testTZ2() {
    const anya_interval = new Algs.AnyaInterval([0, 7], [23, 7], AnyaIntervalType.Closed);
    const left = anya_interval.left;
    const right = anya_interval.right;
    const left_rounded = [Math.floor(left[0]), left[1]];
    const right_rounded = [Math.ceil(right[0]), right[1]];
    ...
    const isOccupied = bruh;

    const coords = [];
    let cur = left;
    while (cur[0] < right[0]) {
      coords.push(cur);
      cur = HorizontalCornerTracer.run(cur, true, bruh, false, false, false, true);
      cur[0] = Math.min(cur[0], right[0]);
    }

    coords.push(right);

    const start_occupied = isOccupied(Utils.topRightCellFromVertex(left)
      || isOccupied(Utils.bottomRightCellFromVertex(left)));
    ...
    for (let i = 1; i < coords.length; i++) {
      ...
      const end = coords[i];
      const interval = new Algs.AnyaInterval(start, end, AnyaIntervalType.Closed);
      if ((i + start_occupied) % 2) {
        intervals.push(interval);
      } else {
        terminals.push(interval);
      }
    }

    console.log(intervals, terminals);
  ...
  makeMap(new_states, new_costs, keep_costs) {
    ...
    for (const [key, value] of Object.entries(new_states)) {
      if (ui_states[key] === undefined)
        throw new Error(
          `ui_states does not have the requested property "${key}"`
        );
      ui_states[key] = value;
    }
    ...
    const new_start = Utils.isCoord(new_states.start)
      ? new_states.start
      : ui_states.start;
    const new_goal = Utils.isCoord(new_states.goal)
      ? new_states.goal
      : ui_states.goal;
    ...
    this.tool_start_x.changeParams(
      new_start[0],
      0,
      ui_states.size[0],
      0.5,
    ...
    this.tool_start_y.changeParams(
      new_start[1],
      0,
      ui_states.size[1],
      0.5,
    ...
    // draw cost and step
    this.tool_draw_cost.changeParams(
      ui_states.draw_cost,
      0,
      Infinity,
      ui_states.draw_step,
  ...
  changeDrawCost(cost) {
    if (ui_states.changing_draw_cost) return;
   ...
   */
  changeStart(start) {
    if (ui_states.changing_start) return;
   ...
   */
  changeGoal(goal) {
    if (ui_states.changing_goal) return;
  ...
  runMode(alg_params) {
    ...
    } else if (alg_params.algorithm === AlgAlgorithm.PRM) {
      ui_states.alg = new Algs.PRM(alg_params);
    } else if (alg_params.algorithm === AlgAlgorithm.RRT) {
      ui_states.alg = new Algs.RRT(alg_params);
    ...
    } else if (alg_params.algorithm === AlgAlgorithm.ThetaStar) {
      // ui_states.alg = new Algs.ThetaStar(alg_params);
      window.alert("Algorithm implementation requires fixing.");
      this.drawMode();
      return;
    } else if (alg_params.algorithm === AlgAlgorithm.Anya) {
      ui_states.alg = new Algs.Anya(alg_params);
    ...
    this.tool_lens.replaceOptions(lens_options, ui_states.alg.default_lens_idx);
    ...
    this.tool_rank.replaceOptions(rank_options, ui_states.alg.default_rank + 1);
  ...
  changeLens(lens_idx) {
    if (ui_states.draw_mode === false) {
      if (ui_states.changing_lens === true) return;
      ui_states.changing_lens = true;

      if (
        !Utils.isFiniteNonNegativeInteger(lens_idx) ||
        lens_idx > ui_states.alg.num_lenses
      )
  ...
  changeRank(rank) {
    if (ui_states.draw_mode === false) {
      if (ui_states.changing_rank === true) return;
      ...
      if (rank >= ui_states.alg.num_ranks)
        throw new Error(
          `rank "${rank}" is larger than number of ranks (${ui_states.alg.numRanks}).`
        );
      ...
      this.tool_step.changeParams(
        step_idx + 1,
        1,
        this.player.numSteps(rank),
        1,
        true
      );
      this.tool_step.label = `Go to step (Alt+5). There are ${this.player.numSteps(
        rank
      )} rank-${rank + 1} steps`;
  ...
  toggleForward() {
    if (this.tool_play_forward.value) this.pause();
    else this.playForward();
  },

  toggleReverse() {
    if (this.tool_play_reverse.value) this.pause();
  ...
  playForward() {
    if (ui_states.draw_mode === false) {
      this.tool_play_reverse.label =
        "Click to rewind (Alt+3). Double-click to go to start (Alt+Comma)";
      this.tool_play_reverse.release();
      if (this.tool_play_reverse.isHovered()) this.tool_play_reverse.setTip();

      this.tool_play_forward.label =
        "Click to pause (Alt+9). Double-click to go to end (Alt+Period)";
      this.tool_play_forward.press();
      if (this.tool_play_forward.isHovered()) this.tool_play_forward.setTip();
  ...
  playReverse() {
    if (ui_states.draw_mode === false) {
      this.tool_play_reverse.label =
        "Click to pause (Alt+3). Double-click to go to start (Alt+Comma)";
      this.tool_play_reverse.press();
      if (this.tool_play_reverse.isHovered()) this.tool_play_reverse.setTip();

      this.tool_play_forward.label =
        "Click to play (Alt+9). Double-click to go to end (Alt+Period)";
  ...
  pxToGrid(px_coord) {
    return [
      px_coord[0] / ui_params.cell_size,
      ui_states.size[1] - px_coord[1] / ui_params.cell_size,
    ];
  },
  ...
  gridToPx(grid_coord) {
    return [
      grid_coord[0] * ui_params.cell_size,
      (ui_states.size[1] - grid_coord[1]) * ui_params.cell_size,
    ];
  },
  ...
  inMapCell(cell_coord) {
    return (
      cell_coord[0] >= 0 &&
      cell_coord[0] < ui_states.size[0] &&
      cell_coord[1] >= 0 &&
      cell_coord[1] < ui_states.size[1]
    );
  },
  ...
  inMapVertex(vertex_coord) {
    return (
      vertex_coord[0] >= 0 &&
      vertex_coord[0] <= ui_states.size[0] &&
      vertex_coord[1] >= 0 &&
      vertex_coord[1] <= ui_states.size[1]
    );
  },
  ...
  serializeVertexEdge(vertex_coord1, vertex_coord2) {
    return (
      vertex_coord1[0] * (ui_states.size[1] + 1) +
      vertex_coord1[1] +
      vertex_coord2[0] * (ui_states.size[1] + 1) +
      vertex_coord2[1]
    );
  },
};