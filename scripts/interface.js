...
class UIPath {
  constructor(start_pos, end_pos, line_radius_x, line_radius_y, class_name) {
    this.ele = document.createElement('div');
    this.start_pos = new Vec(-1, -1);
    this.end_pos = new Vec(-1, -1);
    this.edit.apply(this, arguments);
  }
  edit(start_pos, end_pos, line_radius_x, line_radius_y, class_name) {
    var ele = this.ele;
    if (!this.start_pos.equals(start_pos) || !this.end_pos.equals(end_pos) || this.line_radius_x !== line_radius_x || this.line_radius_y !== line_radius_y) {
      ...
      ele.style.width = ''.concat(length, 'px');
      ele.style.transform = 'rotate('.concat(angle * 180 / Math.PI, 'deg)');
      ele.style.top = ''.concat(yi+length*Math.sin(angle)/2 - line_radius_y, 'px');
      ele.style.left = ''.concat(xi-length*(1-Math.cos(angle))/2 - line_radius_x, 'px');
      this.start_pos = start_pos;
      this.end_pos = end_pos;
      this.line_radius_x = line_radius_x;
      this.line_radius_y = line_radius_y;
    }
    ele.classList = '';
...
class UI {
  ...
  initialise_graphic_handlers() {
    ...
    handlers.new_map = function(input) {
      ...
      var data_validator = data_handlers.validate_map_size;
      if (data_validator(ele_i.value) === false || data_validator(ele_j.value) === false) {
      ...
      if (state.saved == false)
        if (window.confirm('You have not saved the map, do you want to continue?') == false)
          return true;
      // New map, no need to care about saving
      state.saved = true;
      // Clear steps
      steps_obj.steps = [];
      ...
      // update cells window limits
      handlers.update_cells_box();
      
      // put the start and goal
      var xy = data_handlers.map_to_window_cells(new Vec(0, 0), num_i, num_j);
      html_map_obj.start.style.top = xy.y + 'px';
      html_map_obj.start.style.left = xy.x + 'px';
      xy = data_handlers.map_to_window_cells(new Vec(num_i-1, num_j-1), num_i, num_j);
    ...
    handlers.draw_path = function(start_vec, end_vec, class_name) {
      ...
      if (ui_map.paths[ki] === undefined) {
        ...
        ui_map.paths[ki][kf].edit(start_pos, end_pos, self.LINE_RADIUS_X, self.LINE_RADIUS_Y, class_name);
      }
    }
    // Hide a path
            ...
            case 'p': // draw path
              ...
              if (prev_path === undefined) {
                ...
                compiled_action = {
                  ...
                  bck_args : [args[0], args[1], prev_path.class_name]
                }
              }
              // apply the forward step
              compiled_action.fwd_handler.apply(null, compiled_action.fwd_args);
    ...
    handlers.playable = function() {
      return state.play === true && state.step < steps_obj.steps_compiled.length 
    }
    // Play
