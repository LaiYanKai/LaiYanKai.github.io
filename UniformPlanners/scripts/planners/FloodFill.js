class FloodFill extends Planner {
  constructor() {
    super('floodfill', 'FloodFill (BFS)', [
      'blocking',
      'directions',
      'origin',
      'anticlockwise'
  ...
  run() {
    ...
    var step;
    // ==== Inits - Data ====
    var graph = new FloodFill.Graph(this.map.num_i, this.map.num_j);
    this.graph = graph;
    var queue = new FloodFill.Queue();
    // Get the options
   ...
   // Get the search directions
    ...
    this.new_info_list_pane('c', 'Visited (Current) Vertex', 'Put the starting vertex in the Queue. It is programmatically simpler to implement it this way', [['Position', 'Step', 'Parent']]);
    this.new_info_list_pane('n', 'Neighbors', '', [['Direction', 'Position', 'Step', 'State']]);
    this.new_info_list_pane('u', 'Unvisited Queue', '', [['Neighbor', 'Step', 'Parent']]);
    ...
    // add the start vertex into the queue info
    step.insert_list_item('u', 0, [this.start_position.string(), 0, '-']);
    // color the new entry in the queue info
    ...
    while(queue.is_populated()) {
      // ======================== Retrieve Lowest Cost Vertex ================================
      // get the lowest cost vertex from the unvisited queue
      expanded_vertex = queue.retrieve();
      // get its position
      expanded_pos = expanded_vertex.position;
      // get the string
      expanded_pos_string = expanded_pos.string();
      // ==== GUI ====
      // remove the first item in the unvisited info queue
      step.remove_list_item('u', 0);
      // set sim status string
      step.set_info_text('i', '<b>Exploring...</b></br>- Retrieve vertex from queue.')
      // set current vertex status to reflect removal
      step.set_list_description('c', 'Retrieved vertex from queue.');
      // set unvisited vertext status to reflect removal
      step.set_list_description('u', 'Removed first element, because the queue was already sorted'); 
      ...
      step.set_info_text('i', '<b>Exploring...</b></br>- Retrieve lowest cost vertex from open-queue.</br>&ensp;- Check its neighbors');
      ...
      for (var n=0; n<neighbor_nodes.length; n++) {
        ...
        neighbor_vertex.step = expanded_vertex.step + 1;
        // ==== GUI ====
        step.set_list_description('n', 'Neighbor '.concat(neighbor_pos_string, ' is <b>new</b>. Add this to the queue with the current vertex as parent.'));
        // update neighbor information
        step.edit_list_item('n', n, [
          dir_to_string[n], 
          neighbor_pos_string, 
        ...
        step = this.add_step();
        // unhighlight the current queue row
      ...
      // assign the current step as a major step because we are retrieving a new node from the queue
      step.major = true;
      //
      if (path_found === true)
        break;
  ...
  constructor(num_i, num_j) {
    ...
    for (var i=0; i<num_i; i++) {
      ...
      for (var j=0; j<num_j; j++) {
        row[j] = new FloodFill.Vertex(i, j);
      }
      this._v[i] = row;
    }
    this.ni = num_i;
